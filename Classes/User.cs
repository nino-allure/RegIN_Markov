...
namespace RegIN_Markov.Classes
{
    public class User
    {
        ...
        public void GetUserLogin(string Login)
        {
            ...
            {
                MySqlDataReader userQuery = WorkingDB.Query($"SELECT * FROM `user` WHERE `Login` = '{Login}'", mySqlConnection);
        ...
        public void CreateNewPassword()
        {
            if (Login != String.Empty)
            {
                ...
                if (WorkingDB.OpenConnection(mySqlConnection))
                {
                    WorkingDB.Query($"UPDATE `user` SET `Password` = '{this.Password}' WHERE `Login` = '{this.Login}'", mySqlConnection);
                }
                WorkingDB.CloseConnection(mySqlConnection);
